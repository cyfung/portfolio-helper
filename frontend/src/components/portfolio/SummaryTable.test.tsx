...
describe('portfolio value cards', () => {
  ...
  it('shows portfolio, equity-base, and stock-gross values with their own daily percentages', () => {
    ...
    const portfolioChange = screen.getByRole('group', { name: 'Portfolio Value' })
      .querySelector('.portfolio-value-card-change')
    const dayChange = dayChangeRow.querySelector('#total-day-change')
    expect(dayChange?.classList.contains('positive')).toBe(true)
    expect(dayChange?.classList.contains('negative')).toBe(portfolioChange?.classList.contains('negative'))
    expect(dayChange?.classList.contains('after-hours')).toBe(portfolioChange?.classList.contains('after-hours'))
